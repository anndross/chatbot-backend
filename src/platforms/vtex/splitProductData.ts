export function splitProductData(data: any, maxLength: number = 20000): string[] {
    ...
    const addSection = (sectionLabel: string, content: string): void => {
        ...
        if (section.length > maxLength) {
            ...
            if (sectionLabel.startsWith('Variation - ')) {
                warningMessage = `⚠️ A seção de variação ${sectionLabel.replace('Variation - ', '')} foi truncada por exceder ${maxLength} caracteres.`;
            ...
            if (Array.isArray(value) && value.length > 0) {
                ...
                if (firstItem && typeof firstItem === 'object' && Array.isArray(firstItem.sellers) && firstItem.sellers.length > 0) {
                    ...
                    if (firstSeller && typeof firstSeller === 'object' && firstSeller.commertialOffer && typeof firstSeller.commertialOffer === 'object') {
                        Object.entries(firstSeller.commertialOffer).forEach(([coKey, coValue]) => {
                            ...
                            if (coKey === 'Installments' && Array.isArray(coValue)) {
                                ...
                                const filteredInstallments = Object.values(installmentsByPaymentSystem);
                                addSection(`items[0].sellers[0].commertialOffer.${coKey}`, formatValue(filteredInstallments));
                            } else {
                                // Processamento padrão para as demais propriedades
                                addSection(`items[0].sellers[0].commertialOffer.${coKey}`, formatValue(coValue));
                            }
                        });
                    }
                }
    ...
    if (Array.isArray(data.items) && data.items.length > 0) {
        ...
        Object.entries(variationsAggregated).forEach(([variationKey, valuesSet]) => {
