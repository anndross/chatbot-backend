...
export async function addProductEmbeddings(
  productTexts: string[]
): Promise<void> {
  if (vectorStore.memoryVectors.length > 0) return;

  const embeddings: number[][] = await openaiEmbeddings.embedDocuments(
    productTexts
  );

  console.log(
    "🔍 Dimensão do vetor de embedding gerado:",
    embeddings[0].length
  );

  await vectorStore.addDocuments(
    productTexts.map((text: string, index: number) => ({
      pageContent: text,
...
export async function searchRelevantInfo(
  ...
  console.log(
    "\x1b[33m vectorStore - log \x1b[0m",
